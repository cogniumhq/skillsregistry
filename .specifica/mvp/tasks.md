...
- ☐ **T-1.4** Extract `@skillsregistry/domain` — landed in six sub-PRs. Only publish `1.0.0` after (f) so consumers see one coherent surface.
  ...
  - ☐ **T-1.4b** Move `src/resilience/*` (circuit breaker, tenant registry, rate limiter). Least dependent; establishes pattern.
  - ☐ **T-1.4c** Move `src/providers/*` behind `SearchProvider` abstraction. `pgvector-provider.ts` accepts `SqlClient` at construction.
  - ☐ **T-1.4d** Move `src/intelligence/*` (confidence-gate, reranker, deep-search, composition-detector). Depends on providers + adapters.
  - ☐ **T-1.4e** Move `src/composition/*` (fork, copy, compose, extend, lineage). Depends on `@skillsregistry/schema`.
  - ☐ **T-1.4f** Move `src/cognium/scoring-policy.ts` (pure transform only). Circle-IR client stays in mothership.
